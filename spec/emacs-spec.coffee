...
describe 'Emacs', ->
  ...
  describe "emacs:delete-horizontal-space", ->
    it "deletes all horizontal space around each cursor", ->
      EditorState.set(editor, "a [0]\tb c [1]\td")
      atom.commands.dispatch(editorElement, 'emacs:delete-horizontal-space')
  ...
  describe "emacs:backward-kill-word", ->
  ...
  describe "emacs:just-one-space", ->
    it "replaces all horizontal space around each cursor with one space", ->
      EditorState.set(editor, "a [0]\tb c [1]\td")
      atom.commands.dispatch(editorElement, 'emacs:just-one-space')
      expect(EditorState.get(editor)).toEqual("a [0]b c [1]d")
  ...
  describe "emacs:backward-paragraph", ->
    it "moves the cursor backwards to an empty line", ->
      EditorState.set(editor, "aaaaa\n\nbbbbbb")
      editor.moveToBottom()
      atom.commands.dispatch(editorElement, 'emacs:backward-paragraph')
  ...
  describe "emacs:forward-paragraph", ->
    it "moves the cursor forward to an empty line", ->
      EditorState.set(editor, "aaaaa\n\nbbbbbb")
      editor.moveToTop()
      atom.commands.dispatch(editorElement, 'emacs:forward-paragraph')
      expect(editor.getCursorBufferPosition().row).toEqual(1)
  ...
  describe "emacs:exchange-point-and-mark", ->
  ...
  describe 'emacs:kill-whole-line', ->
    it 'kill an entire line at once', ->
      EditorState.set(editor, "aa\nb[0]b\ncc")
      atom.commands.dispatch(editorElement, 'emacs:kill-whole-line')
  ...
  describe 'emacs:append-next-kill', ->
    it 'appending kills', ->
      EditorState.set(editor, '[0]aaa bbb ccc ddd')
      atom.commands.dispatch(editorElement, 'emacs:kill-word')
      atom.commands.dispatch(editorElement, 'emacs:forward-word')
      atom.commands.dispatch(editorElement, 'emacs:append-next-kill')
      atom.commands.dispatch(editorElement, 'emacs:kill-word')
